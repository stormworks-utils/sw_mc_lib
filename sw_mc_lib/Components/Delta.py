...
class Delta(Component):
    def __init__(self, component_id: int, position: Position, input_value: Optional[int]):
    ...
    def from_xml(element: XMLParserElement) -> Delta:
        ...
        assert element.attributes.get('type', '0') == str(ComponentType.Delta.value), f'Not an Delta {element}'
