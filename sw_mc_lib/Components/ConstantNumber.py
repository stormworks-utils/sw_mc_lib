from __future__ import annotations

from sw_mc_lib.Component import Component, INNER_TO_XML_RESULT
from sw_mc_lib.Position import Position
from sw_mc_lib.Types import ComponentType
from sw_mc_lib.XMLParser import XMLParserElement
from sw_mc_lib.NumberProperty import NumberProperty


class ConstantNumber(Component):
    def __init__(self, component_id: int, position: Position, value: NumberProperty):
        super().__init__(ComponentType.ConstantNumber, component_id, position, 0.5)
        self.value: NumberProperty = value

    @staticmethod
    def from_xml(element: XMLParserElement) -> ConstantNumber:
        assert element.tag == "c", f"invalid ConstantNumber {element}"
        assert element.attributes.get("type", "0") == str(
            ComponentType.ConstantNumber.value
        ), f"Not an ConstantNumber {element}"
        obj: XMLParserElement = element.children[0]
        component_id, position, inputs, properties = ConstantNumber._basic_in_parsing(
            obj
        )
        value: NumberProperty = properties.get("n", NumberProperty("0", "n"))
        return ConstantNumber(component_id, position, value)

    def _inner_to_xml(self) -> INNER_TO_XML_RESULT:
        children: list[XMLParserElement] = self._pos_in_to_xml()
        children.append(self.value.to_xml())
        return {}, children
