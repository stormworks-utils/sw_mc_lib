...
class ArithmeticFunction8In(Component):
    def __init__(
        ...
        super().__init__(ComponentType.ArithmeticFunction8In, component_id, position, 2.25)
    ...
    def from_xml(element: XMLParserElement) -> ArithmeticFunction8In:
        ...
        return ArithmeticFunction8In(component_id, position, function, inputs.get(1), inputs.get(2), inputs.get(3), inputs.get(4), inputs.get(5), inputs.get(6), inputs.get(7), inputs.get(8))

    def _inner_to_xml(self) -> INNER_TO_XML_RESULT:
        attributes: dict[str, str] = {'e': self.function}
        children: list[XMLParserElement] = self._pos_in_to_xml({1: self.x, 2: self.y, 3: self.z, 4: self.w, 5: self.a, 6: self.b, 7: self.c, 8: self.d})
